...
>>>>>>> cb703d2bb9360e885a064ec34d3ded1fc7dbfc9b
...
renderPlot({
  ...
  # Adjust data for centered Y-axis and reorder teams by number of wins
  filtered_data <- filtered_data %>%
    mutate(
      adjusted_losses = -l,      # Make losses negative for the left side
      adjusted_wins = w,         # Wins remain positive
      team_id = fct_reorder(team_id, w, .desc = TRUE) # Reorder teams by wins (descending)
    )
  
  # Create the plot
  ggplot(filtered_data, aes(x = team_id)) +
    geom_bar(aes(y = adjusted_losses, fill = "Losses"), stat = "identity", alpha = 0.6) +
    geom_bar(aes(y = adjusted_wins, fill = "Wins"), stat = "identity", alpha = 0.6) +
    labs(
      title = paste("Wins and Losses in", input$selected_decade), 
      x = "Teams Ordered by Wins", 
      y = "Wins (Right) and Losses (Left)"
    ) +
...
-   **Important Takeaways**: Teams are ordered by the number of wins, with the best-performing teams (e.g., "BSN") on the left. Losses (red bars) are negative, and wins (blue bars) are positive, showing a clear contrast.

```{=html}
<!-- -->
```
-   **Why It Matters**: This organization makes it easy to compare team performance at a glance and highlights dominance and struggles.

-   **Surprises**: Some teams have significantly more losses than wins, suggesting they consistently underperformed.

-   **Key Understanding**: The chart provides a clear way to see which teams excelled and which struggled in the 1870s, helping us understand historical team performance trends.
